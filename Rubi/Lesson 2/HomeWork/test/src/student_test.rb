...
class StudentTest < Minitest::Test
  def setup
    ...
    @student = Student.new('Ivanenko', 'Ivan', @valid_date)
  end
  ...
  def test_equality
    same_student = Student.new('Ivanenko', 'Ivan', @valid_date)
    different_student = Student.new('Shevchenko', 'Oleg', '1999-12-31')
  ...
  def test_invalid_date_format
    assert_raises(ArgumentError) { Student.new('Ivanenko', 'Ivan', '01/01/2000') }
  end


  def test_future_date
    future_date = (Date.today + 1).to_s
    assert_raises(ArgumentError) { Student.new('Ivanenko', 'Ivan', future_date) }
  end
  ...
  def test_birthdate_on_boundary
    boundary_student = Student.new('Fransua', 'Illia', '2000-12-31')
